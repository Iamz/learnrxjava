...
public class ComposableListExercises<T> extends ArrayList<T> implements ComposableList<T> {
    ...
    */
    ...
    public static ComposableList<Integer> exercise8() {
        ...
        return newReleases.filter(v -> v.rating == 5.0).map(v -> v.id);
    }
    ...
    public static ComposableList<Integer> exercise9() {
        ...
        movieLists.forEach(list -> {
            list.videos.map(v -> v.id).forEach(id -> allVideoIdsInMovieLists.add(id));
        });
    ...
    public <R> ComposableList<R> concatMap(
        ...
        for (T itemInList : this) {
            projectionFunctionThatReturnsList.apply(itemInList).forEach(r -> results.add(r));
        }
    ...
    public static ComposableList<JSON> exercise12() {
        ...
        return movieLists.concatMap(
                movieList -> movieList.videos.concatMap(
                        video -> video.boxarts.filter(boxArt -> boxArt.width == 150 && boxArt.height == 200).map(
                                ba -> json("id", video.id, "title", video.title, "boxart", ba.url)
                        )
                )
        );
    }
    ...
    */
    public static ComposableList<Integer> exercise16() {
        ...
        return ratings.reduce((ac, it) -> ac > it ? ac : it);
    }
    /*
    ...
    public static ComposableList<String> exercise17() {
        ...
        // returns a list with one item.
        return boxarts.reduce((acc, it) -> it.width * it.height > acc.width * acc.height ? it : acc).map(it -> it.url);
    }
    ...
    */
    ...
    public static ComposableList<JSON> exercise23() {
        ...
        */
        return movieLists.concatMap(
                movieList -> movieList.videos.concatMap(
                        video -> ComposableListExercises.zip(
                                video.boxarts.reduce((a, b) -> a.width * a.height < b.width * b.height ? a : b),
                                video.interestingMoments.filter(m -> m.type.equals("Middle")),
                                (ba, m) -> json("id", video.id, "title", video.title, "time", m.time, "url", ba.url))
                )
        );
    }
